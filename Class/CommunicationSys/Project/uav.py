#!/usr/bin/env python3
"""
uav_localization_pipeline.py
Consolidated pipeline: generate indoor/outdoor users, compute UAV placements,
estimate link SNR/throughput using FSPL, and produce simple plots.
"""
...
# Scenario generation
# -----------------------------
def generate_outdoor_users(n_users=10, area_center=(0.0,0.0), area_radius=150.0, seed=1):
...
# Pipeline
# -----------------------------
def run_pipeline(n_outdoor=10, n_indoor=6, room_size=(12,9), area_radius=150, building_offset=(50,30)):
    outdoor = generate_outdoor_users(n_outdoor, area_center=(0,0), area_radius=area_radius)
    ...
    meta = {'uav_initial': uav_initial.tolist(),
...
def plot_and_save(df, meta, outdoor_pts, indoor_pts, out_dir='outputs'):
    ...
    plt.scatter(meta['uav_initial'][0], meta['uav_initial'][1], marker='x', s=100, label='uav initial')
    plt.scatter(meta['uav_centroid'][0], meta['uav_centroid'][1], marker='^', s=100, label='uav centroid')
    plt.scatter(meta['uav_weighted'][0], meta['uav_weighted'][1], marker='o', s=100, label='uav weighted')
...
def main():
    df, meta, outdoor, indoor_world = run_pipeline(
        n_outdoor=12, n_indoor=8, room_size=(12,9), area_radius=160, building_offset=(50,30)
    )
    print("UAV positions:", meta)
    print("Avg throughputs (Mbps): init={:.2f}, centroid={:.2f}, weighted={:.2f}".format(
        df['th_init_mbps'].mean(), df['th_centroid_mbps'].mean(), df['th_weighted_mbps'].mean()
    ))
    pos_path, th_path = plot_and_save(df, meta, outdoor, indoor_world, out_dir='outputs')
